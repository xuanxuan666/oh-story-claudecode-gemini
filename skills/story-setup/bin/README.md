...
## 写法方法论（通用 · 在技能 references 里，不释放到项目）
通用网文写法方法论（对齐番茄官方教程）统一放在技能 references（writing-craft / dialogue-mastery / character-* / hooks-* / plot-* / opening-design 等），由 Claude（大脑）读取、并在每章写作简报里按需把「本章写法要点」揉给 Gemini。**桥 exe 不带任何方法论、项目里也不释放"写法铁律"文档**——Gemini 靠 Claude 的简报 + 项目文件（细纲 / 上一章 / 本书文风）写正文，Claude 用同一套方法论审校打回。
