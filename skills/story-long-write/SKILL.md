...
7. **正文执行**：
   > **【Gemini 执笔分支（可选，story-setup 开启）】** 若满足激活条件（`.story-deployed` 标 `prose_engine: gemini` 或 `设定/写手.md` 标 `engine: gemini`，且 gemini-bridge 可用，详见 [references/gemini-writer.md](references/gemini-writer.md)），**改按 gemini-writer.md 执行**：写前准备照 1-6 → 拟写作简报+必读清单 → 调 `{bridge} --write [--config {bridge_config}] --project {项目根} --brief {brief}`，并为清单中的每项重复传 `--require {文件或通配}` → 看 stderr 确认 `[✓ 必读覆盖]`（漏读自动打回补读；退出码 2 表示远程 API Key 无效，提示 `{bridge} --login [--config {bridge_config}]`）→ 正文前加 `## 第N章 章名` 落盘 → 直接进入步骤 8（字数验证）。质检（步骤 9-11）按本书文风适配。完成后**跳过下面的 narrative-writer 路径**。
