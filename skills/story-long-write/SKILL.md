...
story-architect 属于高层级结构设计 agent。轻量题材定位优先由主会话完成；只有涉及复杂世界观、多线结构、强反转工程或用户明确要求时，才调用 story-architect。确认选题方向后，如果项目已部署 story-architect agent（按 `.claude/agents/story-architect.md` → `.opencode/agents/story-architect.md` → `.codex/agents/story-architect.toml` 检查），可 spawn `Agent(subagent_type: "story-architect", prompt: "项目目录：{dir}\n任务类型：题材定位\n查询参数：{用户选择的方向+对标信息}")` 辅助题材分析和核心梗设计；Codex 使用同名 `agent_type`。如 agent 不可用，由主线程直接执行。
...
- **设定/世界观/{主题}.md**：世界观框架 + 升级体系设计（参考 worldbuilding-methods.md「世界观框架模板」与「升级体系模板」——时空/社会结构/核心矛盾/特殊系统，及升级体系六步+代价设计；有成长体系的题材必填升级体系）
...
| 世界观 / 升级体系设计 | `references/worldbuilding-methods.md` |
