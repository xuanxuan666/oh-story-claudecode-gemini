...
- **通用写法已内置**：桥自动把 exe 同目录的 `写作铁律.md`（网文写法铁律：小白文 / 话演推动 / 钩子爽点 / 去 AI 味 / 节奏台词配比）拼进 Gemini 系统提示词，**每次调用都教**；**本书 `文风 / 写法自查` 优先于它**（排版空行、……/—— 等冲突以本书为准）。所以简报聚焦本章特定的意图 / 设定红线 / 钩子即可，不必重复通用写法。想改通用写法直接编辑 `story-setup/bin/写作铁律.md`，不用重编 exe。
