# gemini-writer.md：Gemini 执笔（正文交给 Gemini 桥）

> 「正文执行」步骤的 **Gemini 执笔分支**（可选，由 story-setup 开启）。Claude 当大脑（选料 / 拟简报 / 审校 / 质检 / 追踪），Gemini 当枪手（用只读文件工具自己读项目文件、写正文）。接缝只在正文这一步——大纲 / 设定 / 追踪 / 质检管道全不变。桥由 story-setup 随技能预编译打包（`story-setup/bin/gemini-bridge.exe`）并完成浏览器登录，见 story-setup 第 2.8 节。

## 一、何时走本流程（激活条件）
1. 项目 `.story-deployed` 标 `prose_engine: gemini`（story-setup 选“Gemini 执笔”时写入），**或** `设定/写手.md` 标 `engine: gemini`；**且**
2. 存在可用的 **gemini-bridge**（story-setup 已随技能打包 `story-setup/bin/gemini-bridge.exe` 并登录；框架依赖版，需目标机装 .NET 10 运行时）：路径按序取 `设定/写手.md` 的 `bridge:` → 环境变量 `GEMINI_BRIDGE` → `.story-deployed` 的 `gemini_bridge:`。
- 未开启 或 桥不存在 → **回退**原 narrative-writer / 主线程路径，本文件不适用。
- `设定/写手.md`（放项目根 `设定/` 下）可选，用于覆盖模型 / 文风适配 / 必读模板（模板见文末）。

## 二、分工铁律
- **Claude 管计划与判断**：写前准备（状态筛选 / 模块召回 / 文风召回 / 意图确认）照 SKILL.md Phase 4 原样做，产出一份**自足写作简报 + 必读文件清单**。
- **Gemini 管文笔与自取料**：桥让它用 `read_file` / `list_directory`（只读、锁死项目目录）自己读细纲 / 上一章 / 文风 / 对标原文，写出正文。
- **笨模型要管紧**：简报要具体；必读用 `--require` 强制；正文永远经 Claude 质检才落盘，绝不裸奔。
- **通用写法在项目里（不再进 Gemini 系统提示词）**：story-setup 安装时把《写法铁律》释放到项目 `设定/写作铁律.md`；**每章 `--require` 必带上它**，让 Gemini 自己 read_file（文末「十、动笔前最后叮嘱」是核心自检清单）。**本书 `文风 / 写法自查` 优先于它**（排版空行、……/—— 冲突以本书为准）。简报聚焦本章特定的意图 / 设定红线 / 钩子即可，不必重复通用写法。想改通用写法就编辑项目里的 `设定/写作铁律.md`——**每本书可各自定制**，不用重编 exe。

## 三、单章执行步骤
1. **写前准备**：照 SKILL.md Phase 4 步骤 1-6（状态筛选、情绪 / 节奏 / 文风召回、意图确认、标题预检）。**关键——召回本书文风**：读 `设定/文风.md`（无则 `设定/写法自查.md`，再无则对标 `文风.md`），确定本书排版 / 句长 / 话演 / 标点禁忌。例：番茄轻小说＝段间空行、对话独立成段、允许 ……/——、话演≥60%、少想多做、禁对仗。
2. **拟写作简报**（落一个临时 brief 文件，模板见文末）：① 本章意图（情绪 / 节奏 / 核心事件 / 钩子 / 爽点，来自意图确认＋细纲骨架）② **文件清单**（本章必读的项目相对路径）③ 硬约束（字数、本书文风、设定红线、章首 / 章尾钩子）④ 输出要求（不输出标题、只正文）。简报只放 Claude 的**指令与清单**，不必内联文件正文——Gemini 会自己读。
3. **调桥**（`{bridge}` = 激活条件里解析到的 exe 路径；`--require` = 简报文件清单，逗号分隔）：
   ```
   {bridge} --write --project "{项目根}" --brief-file "{brief}" --require "{清单}" [--model {pro|flash}]
   ```
   `stdout` = 正文；`stderr` = 工具轨迹 + `[读取]` / `[✓ 必读覆盖]` / `[⚠ 漏读必读]` + `[模型 …]`。
   - **⏱ 必须用长超时调桥**：一章要让 Gemini 读多个文件 + 高思考写 3000-4000 字，通常 **3-5 分钟**。用 Bash 调桥时把**超时设到 600000ms（10 分钟）**——默认 2 分钟会被掐断，这就是「调用 Gemini 老是超时」的主因。桥内部已对网络瞬断自动退避重试（最多 3 次）。
   - **📋 必读清单（--require）用通配更稳**：上一章写 `正文/第{N-1}章_*.md`（桥支持 `*` 通配，自动匹配真实文件名），别写死可能已改的章节标题；清单里磁盘上不存在的文件，桥会自动跳过、不再死追空转。
   - **模型选择**（取 `设定/写手.md` 的 `model:`，缺省 pro）：`--model pro` = Gemini 3.1 Pro (High)（文笔最好）；`--model flash` = Gemini 3.5 Flash (High) = `gemini-3-flash-agent`（快、省额度）。**两档思考等级都是 high**（桥对非 2.5 模型统一注入 `thinkingLevel=high`）。正文默认 pro；赶进度 / 走量 / 先出草稿可用 flash。
4. **验监督**：看 `stderr`——`[✓ 必读覆盖]` 才算达标；`[⚠ 漏读必读]`（补读 2 次仍漏）则据情形重跑或补简报再跑。**退出码 2（缺登录）→ 提示用户重跑 `{bridge} --login` 走浏览器授权**后再来。
5. **落盘**：正文前加 `## 第N章 {章名}`，写入 `正文/第XXX章_章名.md`。
6. **质检（按本书文风适配 —— 关键，别机械套默认规则）**：
   - **始终跑** `node scripts/check-degeneration.js --check <正文>`（复读 / 截断 / 元信息 / 工程词，模型无关兜底）＋ 禁用词扫描 ＋ 正文元信息扫描 ＋ Python 字数核对。
   - `node scripts/check-ai-patterns.js --check <正文>` **报告模式**：默认体裁按原规则改 ①②；**番茄 / 空行体裁忽略 `——`、碎句 的命中**（本书允许），只改真正的 `不是X而是Y` 等 AI 句式。
   - `scripts/normalize-punctuation.js`：**仅默认体裁跑**；**番茄 / 空行体裁跳过**（它会清掉本书要的 ……/——/段间空行）。
   - 由**主会话**对落盘文件跑（桥不跑脚本）。
   - **⭐ 写法铁律审校（防「读了铁律不照做」——最关键的一步）**：脚本只抓 AI 句式 / 退化，抓不到「没照写法铁律写」。主会话必须**人读一遍正文**，对照 `写作铁律.md` + 本书文风逐条核：① 话演率够不够（≥60%，扫一眼对话＋动作占比，别整章内心独白）？② 有没有大段内心戏 / 自我解说旁白（「他心里那本账 / 他知道 / 他明白 / 她只觉得 / 越是…越是」）？③ 章尾是否卡在钩子上、有没有「他终于明白 / 这一夜注定无眠」式升华？④ 有没有排比对仗、条理词（首先 / 其次 / 综上 / 这意味着）？⑤ 句子长短错开还是通篇匀速？⑥ 同一小动作（挑眉 / 眯眼 / 勾唇）是否一章超 2 次？⑦ 爽点有没有先铺危机再释放、对手当场破防？命中即列成「违反清单」，进步骤 7 打回。
7. **打回重写**：
   - 确定性 AI 模式（脚本命中）→ 主会话直接改。
   - **写法铁律违反（步骤 6 的「违反清单」）/ 内容 / 设定级问题** → 把违反清单写进简报（新增一段「上一稿这些地方违反了写法铁律，必须改：<逐条列具体位置+怎么改>」），**重调桥让 Gemini 针对性重写**（最多 2 次）。⚠ **别指望 Gemini 自觉——它读了《写法铁律》也常不照做**；可靠性全靠这一步「Claude 审校 + 打回重写」兜死，跟质检脚本一个道理。
8. **更新追踪**：照 SKILL.md 原样（伏笔 / 时间线 / 角色状态 / 上下文）。

## 四、写作简报模板
```
# 第{N}章 写作简报（Claude 主编拟）
你要写《{书名}》第 {N} 章。动笔前必须用 read_file 逐一读完下面清单，严格照着写。

## 一、动笔前必读（read_file 逐个读）
- 大纲/细纲_第{N}章.md — 本章施工图，逐条落实
- 正文/第{N-1}章_*.md — 上一章，开头承接它
- 设定/写作铁律.md — 通用网文写法准则（文末『十、动笔前最后叮嘱』必看）
- 设定/文风.md 或 设定/写法自查.md — 本书文风铁律（与写作铁律冲突时以本书为准）
- 追踪/角色状态.md、追踪/伏笔.md — 本章相关角色 / 待回收伏笔
- {本章相关的角色卡 / 对标原文锚点 / 世界观设定}

## 二、本章意图
{情绪 + 节奏 + 核心事件 + 章首/章尾钩子 + 爽点（来自意图确认与细纲骨架）}

## 三、硬约束（违反即回炉）
- 字数 {目标} 左右（按细纲情节点字数预算分配）
- 文风：{本书排版 / 句长 / 话演 / 标点禁忌，来自文风召回}
- 设定红线：{本章一致性红线，来自细纲}
- 章首钩子 / 章尾钩子：{细纲}

## 四、输出
- 不要输出章节标题（标题由 Claude 加），只输出正文本身；不要任何说明 / 前言 / 点评 / “我写好了”之类的话。
```

## 五、兜底
- 缺登录（退出码 2）→ 提示用户重跑 `{bridge} --login`（浏览器授权 Antigravity），登录后重跑。
- 桥不可用 / 缺 .NET 10 运行时 → 回退原 narrative-writer / 主线程路径，正常写作（并提示装 .NET 10 运行时或 `/story-setup` 重新配置 Gemini 执笔）。
- Gemini 漏读 / 凭空编 → `--require` + 监督闸已强制补读；仍漏则审校时对照细纲 / 设定，命中即回炉。

## 六、`设定/写手.md` 配置模板（可选，放项目根 设定/ 下）
```
engine: gemini            # 或 claude（关闭 Gemini 执笔、回退原路径）
bridge: <gemini-bridge.exe 绝对路径>   # 留空则用 GEMINI_BRIDGE 或 .story-deployed 的 gemini_bridge
model: pro                # pro = 3.1 Pro High（文笔最好，默认）/ flash = 3.5 Flash High（快）；两者思考都是 high。也可填完整 id 如 gemini-pro-agent
排版风格: 番茄轻小说       # 或 默认；决定质检适配
质检适配: 允许 ……/——/段间空行；跳过 normalize-punctuation
必读清单模板: 大纲/细纲_第{N}章.md, 正文/第{N-1}章_*.md, 设定/写作铁律.md, 设定/写法自查.md, 追踪/角色状态.md, 追踪/伏笔.md
```
