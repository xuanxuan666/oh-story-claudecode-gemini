...
2. **逐章执行**（以下每步在每章循环内执行）：
   ...
   - 写正文（**若启用 Gemini 执笔**：按 [gemini-writer.md](gemini-writer.md) 调 `{bridge} --write [--config {bridge_config}] --project {项目根} --brief {brief}`，为清单每项重复传 `--require`，看 stderr 确认 `[✓ 必读覆盖]`；否则主会话 / narrative-writer 原路径）→ **字数验证（优先 Python 字符统计，`wc -m` 仅作 Unix 备选，< 目标90%则对照情节点字数预算定位欠账密点、一次性重写到配额，不挤牙膏反复回炉；> 章目标×1.1 则压过场/合并疏点收敛；90% 是放行下限非目标，理想落在 [章目标, 章目标×1.1]）** → 检查钩子/爽点 → **正文元信息扫描** → 禁用词扫描
