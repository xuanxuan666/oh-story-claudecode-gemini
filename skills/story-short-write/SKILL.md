...
> **【Gemini 执笔分支（可选，story-setup 开启）】** 若启用 Gemini 执笔（`.story-deployed` 标 `prose_engine: gemini` 或 `设定/写手.md` 标 `engine: gemini`，且 gemini-bridge 可用，激活条件与流程见 [../story-long-write/references/gemini-writer.md](../story-long-write/references/gemini-writer.md)）：短篇同样调 `{bridge} --write --project {项目根} --brief-file {brief} --require {文件清单}`——把本篇/本批的写作简报（小节大纲、题材风格包、已写小节摘要、`正文.md` 尾部衔接句、本篇文风与 short-format 格式硬约束）+ 必读清单交给桥，Gemini 自读后写入 `正文.md`，`stderr` 确认 `[✓ 必读覆盖]`，再按本篇文风质检落盘。**否则按下面原路径。**
