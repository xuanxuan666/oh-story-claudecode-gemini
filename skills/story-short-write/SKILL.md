...
> **【Gemini 执笔分支（可选，story-setup 开启）】** 若启用 Gemini 执笔（`.story-deployed` 标 `prose_engine: gemini` 或 `设定/写手.md` 标 `engine: gemini`，且 gemini-bridge 可用，激活条件与流程见 [../story-long-write/references/gemini-writer.md](../story-long-write/references/gemini-writer.md)）：短篇同样调 `{bridge} --write [--config {bridge_config}] --project {项目根} --brief {brief}`，并为每个必读文件重复传 `--require`。把本篇/本批写作简报、已写小节摘要、`正文.md` 尾部衔接与格式硬约束交给桥，确认 stderr 的 `[✓ 必读覆盖]` 后再按本篇文风质检落盘。**否则按下面原路径。**
