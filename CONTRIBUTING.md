...
- repo-local skills：`.agents/skills/*/SKILL.md` 是由 `scripts/generate-codex-skill-shims.py` 从 `skills/*/SKILL.md` 生成的轻量适配入口。Codex 扫描它们后再读取 `skills/` 真源，references/scripts/assets 不复制。这样 Windows `core.symlinks=false` 也不会把发现路径检出成失效文本文件。OpenClaw 仍可原生扫 workspace `skills/`。
- plugin distribution：仓库根 `.codex-plugin/plugin.json` 将 `./skills/` 打包为一个 Codex 插件；manifest 的 `name` 必须与仓库目录名一致，`version` 必须与 `skills/story/VERSION` 同步。
...
python scripts/generate-codex-skill-shims.py
bash scripts/check-codex-adapter.sh
bash scripts/test-codex-hooks.sh
```

macOS/Linux 只有 `python3` 时用 `python3` 替换上述 `python`；Windows 只有 Python Launcher 时用 `py`。守卫脚本自身会按 `python3 → python → py` 探测。
